...
Send a personal **connection token** as a bearer token in the `Authorization` header:

```
Authorization: Bearer <token>
```

Get a token from **Settings → Connect Claude** (the same token used for MCP and the
Artifacts API). Every call runs as the token's owner — you only ever see and modify
your own to-dos. Revoke a token anytime in Settings to cut off access.
