...
```typescript
const MAX_TOOL_TURNS = 16  // Slack @mentions get chat-sized runway for complex multi-step tasks
const MAX_TOKENS = 8000    // Per-completion output budget
const WALL_CLOCK_MS = 130_000  // Stop 20s before platform kills the worker (130s default, configurable via SLACK_MAX_WALL_MS)
...
// Agent loop with wall-clock guard
messages = [system, user_content]
result = ''
const startedAt = Date.now()
for turn in 0..MAX_TOOL_TURNS:
  // Finalize before platform timeout: post what we have rather than dropping the reply
  if (Date.now() - startedAt > WALL_CLOCK_MS):
    if (!result) result = 'This took longer than I could spend on it — try narrowing the request.'
    break
  
  out = orComplete(messages, tools, maxTokens: MAX_TOKENS, ...)
...
## Proactive posting: `send_slack_message` builtin

The `send_slack_message` builtin (handler in `_shared/builtins.ts`) is the inverse of the reactive @mention path: agents, scheduled runs, chat, and webhooks can now initiate posts to Slack instead of only replying to mentions.

**Implementation:**
- Validates channel (required, either ID or name) and message text (required)
- Rate-limits: 30 posts per rolling hour workspace-wide (`activity_log` type `slack.sent`)
- Reads the bot token from Vault via `read_slack_secrets()` (service-role-only)
- Calls `postSlackMessage(botToken, channel, text, threadTs?)` from `_shared/slack.ts`
- Logs `slack.sent` on success, `slack.error` on Slack API failure
- Returns the result as a human-readable string

**Access control** (like `send_email`):
- Exfiltration-capable tool — gated by admin activation (default inactive in migration 0107)
- Agent `tool_ids` scoping applies
- Webhook `allow_tools` deterministic gate applies
- Every post logs to activity feed for auditing

**Input schema:**
```json
{
  "channel": "C0123456 or #general",
  "text": "The message to post",
  "thread_ts": "ts of parent message (optional, for replies)"
}
```

**Seeding** (migration 0107): The builtin is created with `is_active = false` so an admin must turn it on once Slack is connected.
...
- **Wall-clock timeout** - The bot stops the tool loop and posts a reply 20 seconds before the platform kills the worker (130 seconds by default, overridable with `SLACK_MAX_WALL_MS`). This ensures a response is posted rather than timing out with no output.
