...
The Slack integration lets the workspace bot join Slack channels and answer @mentions using collection context - the same architecture as [webhooks](/docs/building/webhooks) but triggered by Slack events instead of HTTP calls.
...
- **`send_slack_message` builtin** - agents can proactively post into bound rooms ("ambient" behavior, like scheduled email)
- **In-channel commands** - `/slash` commands to manage bindings without leaving Slack
- **Memory** - file notable channel messages back into the bound collection so the bot "remembers"
