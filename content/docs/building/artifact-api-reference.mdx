...
| `visibility` | no | string | `private` (default), `workspace`, `unlisted`, or `public`. Only `unlisted` and `public` mint a `public_slug`. |
...
Visibility options and their effects:

- **private**: only the owner sees the artifact. No external link.
- **workspace**: all team members can see and edit. No external link.
- **unlisted** or **public**: mints a `public_slug` for external sharing.
