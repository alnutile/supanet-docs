...
- **Also save it to a table** — each message deterministically populates a row in one of your tables. Map message fields (sender, subject, body, received date) to columns using **dot-path tokens** like `{{event.data.subject}}` and `{{event.data.from_address}}`. No model is involved, so this is purely deterministic — the same as a webhook's direct-table mode.
