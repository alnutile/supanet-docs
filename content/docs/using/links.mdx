...
If your workspace admin has connected a Dropbox account in **Settings → Dropbox**, Dropbox links automatically fetch richer metadata:

- **File name** — the actual Dropbox file name instead of a generic title.
- **Thumbnail preview** — a visual thumbnail for image files (JPG, PNG, etc.).
- **File details** — file type and modified date.

This works for both public Dropbox share links and personal Dropbox URLs. The integration is transparent — just paste a Dropbox URL as you would any other link and SupaNet fetches enhanced metadata automatically.
